...
public class Tesista implements Runnable {
	...
	private int number; //RANDOM NUMBER OF GS COMPUTER
	...
	public void run() {
		for(int i=0; i<nAccess;i++) {
			...
			try {
				lab.AccessQueue("Tesista",Thread.currentThread(),this); //ENTER THE QUEUE
			...
			lock.lock();
			try {
				while(this.access==0) { //CHECK IF THE GS CAN ENTER
					PCnotAvilable.await();
				}
				this.access = 0;
